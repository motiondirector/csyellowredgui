...
public class ConnectFourGUI
{
	...
	/**
	 * Constructs a GUI for the Connect 4 Game
	 */
	...
	public static void main(String[] args)
	{
		...
		else
		{
			try
			{
				...
				{
					System.err.println("Number of required connections must be between " + CONNECTIONS_MIN + " and "
							+ CONNECTIONS_MAX);
	...
	/**
	 * Goes through a single turn of the game and inserts a piece into the board
	 * 
	 * @param theX the position that the user clicked on
	 */
	private void turn(int theX)
	{
		...
		if (c4.insert(currentPlayer, clickedCol))
		{
			...
			if (c4.hasWin())
			{
				JOptionPane.showMessageDialog(frame, currentPlayer + " player wins", "Game finished",
	...
	private void initComponents()
	{
		...
		uiPanel.addMouseMotionListener(new MouseMotionListener()
		{
			...
			/**
			 * updates top "button" x position to match mouse movement whiting
			 * the panel
			 */
		...
		saveBtn.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent event)
			{
				try
				{
					...
					{
						JOptionPane.showMessageDialog(settingsWindow, "Rows must be between " + GRID_MIN + " and "
								+ GRID_MAX);
					}
					else if (newColumns > GRID_MAX || newColumns < GRID_MIN)
					{
						JOptionPane.showMessageDialog(settingsWindow, "Columns must be between " + GRID_MIN + " and "
								+ GRID_MAX);
					}
					else if (newConnections > CONNECTIONS_MAX || newConnections < CONNECTIONS_MIN
							|| newConnections > Math.min(newRows, newColumns))
					{
						JOptionPane.showMessageDialog(settingsWindow, "Connections must be between " + CONNECTIONS_MIN
								+ " and " + CONNECTIONS_MAX + "\n"
